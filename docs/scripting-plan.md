...
> For what actually exists, see [architecture.md](architecture.md) and
...
1. A Zig-compiled **engine** (closed source): launcher plus injected runtime that
...
5. **Tooling**: static checks for Lua mods, meaning performance lints, dependency
   graphs and API validation, all runnable without launching the game.

Target authoring experience: a whole side-application in Lua, with performance
monitors, in-game overlays and in-game settings screens, and the engine as the
platform.
...
- **The launcher is a native binary** for the user's OS (Linux by default),
  not a Windows exe. Its job is to find the install, drive Proton and arrange
  injection, all of which happens outside the game process, so running it under
  Wine would only add indirection. It starts the Windows game the way Steam does,
...
- This repository stays the open half (tooling, SDK stubs, docs, the offline
  ...
  ship stripped binaries. Decide the split at milestone E0, because
  retrofitting privacy after code is public is impossible.
...
- **Offline enforcement**: a hard rule. The runtime never attaches to a
...
- Per-mod VM instance: one misbehaving mod cannot corrupt another's state,
  and the engine can kill or reload a single mod.
- Sandbox: `base`/`table`/`string`/`math` only; no `io`, `os`, `require`,
  no loading new chunks. File and OS needs go through vetted SDK modules instead.
- Budgets, enforced per event dispatch via `lua_sethook` instruction counting
  ...
  because a slow overlay script must never become a game stutter. This is also
  what makes the perf tooling (below) truthful: the same budget model, offline.
...
The example applications fall out of these directly: a performance monitor is
`perf` plus `ui`; in-game settings are `ui` plus `store`; overlays are `ui`
plus `hooks`.

### Mod format

One script is one mod. A table at the top declares identity and intent:
...
- `ermod check <mod.lua>` sandbox-loads the mod and validates the manifest,
  param and field names, and permission declarations. Static; no game needed.
- `ermod graph <mods-dir>` builds a dependency and conflict graph: which params,
  rows, fields and events each mod touches. It flags conflicting writes across
  mods before anyone launches the game (the same conflict rule as the offline
  pipeline, lifted to the whole mod set).
- `ermod perf <mod.lua>` runs handlers against recorded or synthetic event
  traces under the instruction-count budget model, and reports cost per event and
  the hot paths. An honest pre-flight, not a promise: real in-game cost is
  what `perf` and the engine's per-mod accounting report.
- `ermod stubs` emits Lua type stubs (LuaLS annotations) for the SDK, so
...
### E0. Repo split and feasibility spike *(gate for everything else)*

- Split: the engine goes to a private repo; this repo keeps SDK stubs, tooling,
  docs and the offline pipeline.
- Spike: a Zig launcher starts Elden Ring under Proton with EAC off, and injects
  a Zig DLL that logs to a file and installs **one** AOB-scanned hook that
  fires. Kill or proceed on: game stable, hook fires, dev loop bearable.

### E1. Offline Lua (the former Phase 1, unchanged in substance)

Lua vendored into `ermod`; the `params` module against the offline pipeline;
the sandbox; conflict-checked patch recording; `level60` ported as the reference
...
The hook framework (AOB scanner, trampolines, signature data file, version
gate), per-mod VMs with budgets, the `hooks` and `log` modules, and mod lifecycle
(load at launch, kill on misbehaviour). Launch mods (`run_at = "launch"`)
and the first event mods work in-game.
...
### E4. UI and the platform modules

The DX12 present hook, vendored cimgui, and the `ui`, `perf` and `store` modules.
The three showcase mods get built as SDK validation and shipped as examples: a
performance monitor, an in-game settings screen, and a gameplay overlay.

### E5. Developer experience round-out

`ermod graph`, `ermod perf`, `ermod stubs`; mod hot-reload in-game (edit a
script and the engine reloads that VM, the iteration loop that makes Lua authoring
actually pleasant); a documented signature-update release process.
...
- **Game patches** invalidate signatures and param layouts. Mitigations:
  the signature data file (a data update, not a binary release), the version gate
  (graceful disable), and paramdef identity checks (already in place).
- **Closed engine against community trust.** An injected closed-source binary
  asks for real trust. Mitigations: an open SDK, tooling and docs, a per-mod
