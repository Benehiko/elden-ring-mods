...
ermod-engine dev apply "$GAME/regulation.bin" mod/regulation.bin my_mod.lua
```
...
Event mods are **in-game only**. `ermod-engine dev apply` refuses one rather than
...
Full signatures live in `stubs/ermod.lua` (generated by `ermod-engine dev stubs` and
...
`ermod-engine dev show <regulation.bin> <row-id>` prints the field names a row actually
...
`ermod-engine dev perf` before shipping.
...
```sh
ermod-engine dev check my_mod.lua                    # would it load in-game?
ermod-engine dev perf  my_mod.lua                    # what does it cost per event?
ermod-engine dev apply "$GAME/regulation.bin" mod/regulation.bin my_mod.lua
```
...
```
$ ermod-engine dev perf examples/overlay.lua --frames 120 --runes 5 --deaths 1
...
```
$ ermod-engine dev perf examples/level60.lua --regulation "$GAME/regulation.bin"
...
```
$ ermod-engine dev apply "$GAME/regulation.bin" mod/regulation.bin examples/level60.lua
...
frame; `ermod-engine dev img` measures it, so "did my overlay draw?" is a shell test
rather than a squint:

```sh
ermod-engine dev img stat shot.png --rect 20,20,420,260   # mean colour, lit coverage, distinct colours
ermod-engine dev img diff before.png after.png            # changed pixels + bbox; exit 1 if nothing changed
```
