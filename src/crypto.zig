...
/// Returns freshly allocated [IV][ciphertext]. Pass `iv` for deterministic
/// output (tests); null uses a random IV.
pub fn encrypt(allocator: std.mem.Allocator, key: [32]u8, data: []const u8, iv: ?[block_size]u8) ![]u8 {
    ...
    const iv_bytes: [block_size]u8 = iv orelse blk: {
        var buf: [block_size]u8 = undefined;
        var filled: usize = 0;
        while (filled < buf.len) {
            const n = std.os.linux.getrandom(buf[filled..].ptr, buf.len - filled, 0);
            if (std.os.linux.errno(n) != .SUCCESS) return error.EntropyUnavailable;
            filled += n;
        }
        break :blk buf;
    };
